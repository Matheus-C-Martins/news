<template>
  <main id="sports-page" class="news-container">
    <div class="header">
      <h1><fa icon="fa-solid fa-medal" /> Sports News</h1>
      <p class="subtitle">Latest updates from around the sports world</p>
    </div>
    
    <!-- Loading State -->
    <div v-if="isLoading" class="loading-state">
      <div class="spinner"></div>
      <p>Loading sports articles...</p>
    </div>
    ...
    <div v-else-if="articles.length === 0" class="no-results">
      ...
      <p>Check back later for more sports news</p>
    </div>
...
const loadArticles = async () => {
  ...
  try {
    const data = await fetchNewsByCategory('sports', currentPage.value)
    
    if (data.status === 'error') {
      error.value = data.message || 'Failed to fetch sports articles.'
      articles.value = []
